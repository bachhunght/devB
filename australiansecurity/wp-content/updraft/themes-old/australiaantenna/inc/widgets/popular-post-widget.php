...
class category_posts extends WP_Widget {
    function category_posts() {
        /* Widget settings. */
        $widget_ops = array( 'classname' => 'category_posts', 'description' => 'Displays the post image and excerpt from a selected category **Does not work on singlepost sidebar**'.'wp-rockPalace' );

        /* Create the widget. */
        $this->WP_Widget( 'category_posts-widget', __('CC Popular Posts Widget', 'wp-rockPalace'), $widget_ops, '' );
    }
    ...
    function widget( $args, $instance ) {
        ...
        $pc = new WP_Query("cat=" . $category_id . "orderby=comment_count&showposts=".$number."&nopaging=0&post_status=publish&ignore_sticky_posts=1&post_type=post");
...
<div class="widgett">
    ...
    <div class="details2"><?php echo $no_comments; ?> Comments</div>
</div>
