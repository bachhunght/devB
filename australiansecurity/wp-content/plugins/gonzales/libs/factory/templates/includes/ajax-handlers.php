 ...
 *
 * @param Wbcr_Factory723e1da7c1403346cdca8f259c016fc4_456_Plugin $plugin_instance
 *
 * @since 2.3.0
 *
 */
function wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107_subscribe($plugin_instance)
{
	...
	if( empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL) ) {
		wp_send_json_error(['error_message' => __('You did not send your email address or it is incorrect!', 'wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107')]);
	}

	if( empty($group_id) ) {
		wp_send_json_error(['error_message' => __('Group ID is empty!', 'wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107')]);
	}
	...
	wp_send_json_error(['error_message' => __('Unknown error while trying to subscribe to newsletter.', 'wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107')]);
 ...
 *
 */
function wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107_check_license($plugin_instance)
{
	...
	if( empty($action) || !in_array($action, ['activate', 'deactivate', 'sync', 'unsubscribe']) ) {
		wp_send_json_error(['error_message' => __('Licensing action not passed or this action is prohibited!', 'wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107')]);
		die();
	}
		...
		switch( $action ) {
			case 'activate':
				if( empty($license_key) || strlen($license_key) > 32 ) {
					wp_send_json_error(['error_message' => __('License key is empty or license key too long (license key is 32 characters long)', 'wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107')]);
				} else {
					$plugin_instance->premium->activate($license_key);
					$success_message = __('Your license has been successfully activated', 'wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107');
				}
				break;
			case 'deactivate':
				$plugin_instance->premium->deactivate();
				$success_message = __('The license is deactivated', 'wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107');
				break;
			case 'sync':
				$plugin_instance->premium->sync();
				$success_message = __('The license has been updated', 'wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107');
				break;
			case 'unsubscribe':
				$plugin_instance->premium->cancel_paid_subscription();
				$success_message = __('Subscription success cancelled', 'wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107');
				break;
		}
	} catch( Exception $e ) {
