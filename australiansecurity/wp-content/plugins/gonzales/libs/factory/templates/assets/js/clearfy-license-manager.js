...
jQuery(function($) {
	...
	$(document).on('click', '.wcl-control-btn', function() {
		...
		for( i = 0; i < allNotices.length; i++ ) {
			$.wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107.app.hideNotice(allNotices[i]);
		}
		...
		$.ajax(ajaxurl, {
			...
			success: function(response) {
				...
				if( !response || !response.success ) {
					...
					if( response.data ) {
						...
						noticeId = $.wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107.app.showNotice('Error: [' + response.data.error_message + ']', 'danger');
				...
				if( response.data && response.data.message ) {
					noticeId = $.wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107.app.showNotice(response.data.message, 'success');
					allNotices.push(noticeId);
			...
			error: function(xhr, ajaxOptions, thrownError) {
				...
				var noticeId = $.wbcr_factory_templates_204e5d1397aaf784fcbb05c582936ff1_107.app.showNotice('Error: [' + thrownError + '] Status: [' + xhr.status + '] Error massage: [' + xhr.responseText + ']', 'danger');
