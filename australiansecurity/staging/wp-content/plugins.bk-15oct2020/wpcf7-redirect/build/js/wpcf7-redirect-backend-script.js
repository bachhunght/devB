...
(function ($) {
	function Wpcf7_redirect_admin() {
		...
		this.setparams = function () {
			...
			this.close_popup_selector = '.wpcfr-close-popup';
		...
		this.init_popups = function () {
			var _this = this;
			$(document.body).on('change', '.field-wrap-popup-template select', function () {
				_this.show_loader($(this).parents('.action-container'));
				params = {
					'action_id': _this.get_block_action_id($(this)),
					'template': $(this).val()
				};
				_this.make_ajax_call('wpcf7r_get_action_template', params, 'after_ajax_call');
			});

			$(document.body).on('change', '.template-field input', function () {
				var $action_container = $(this).parents('.action-container');
				var $preview_iframe = $action_container.find('.preview-iframe');
				var url = $preview_iframe.attr('src');
				var template_fields = $action_container.find('.template-field');
				_this.show_loader($preview_iframe.parents('.field-wrap').first());
				$.each(template_fields, function (k, v) {
					var field_name = $(v).find('input').data('name');
					var field_value = $(v).find('input').val();
					if (typeof field_name != 'undefined') {
						url = _this.replace_query_var(url, field_name, encodeURIComponent(field_value));
					}
				});
		...
		this.register_action_hooks = function () {
			...
			//close popup
			$(document.body).on('click', this.close_popup_selector, this.close_popup.bind(this));
		...
		this.close_popup = function (e) {
			$('.wpcfr-popup-wrap').fadeOut(function () {
				$(this).remove();
			});
		}
		...
		this.after_ajax_call = function (e, params, response, action) {
			 ...
			 * @param  {[type]} [action== 'wpcf7r_get_action_template'] [description]
			 * @return {[type]}           [description]
			 */
			if ('wpcf7r_get_action_template' === action) {
				$('[data-actionid="' + params.action_id + '"] .hidden-action').slideUp(function () {
					$(this).html(response.data.action_content);
					$.each($('.wp-editor-wrap textarea', this), function () {
...
function disply_updates_marks() {
    var update_count = $('.extensions-list .has-update').length;
    if (update_count) {
        $('li#extensions-panel-tab a').append(' <span class="update-plugins wpcf7r-update-extensions"><span class="plugin-count">' + update_count + '</span></span>');
    }
}
...
function extension_update_handler() {
    $('.extensions').on('click', '.promo-box .btn-update', function (e) {
        e.preventDefault();
        $extension = $(this).parents('.promo-box');
...
function activate_serial_handler() {
    $('.extensions').on('click', '.promo-box .btn-activate', function (e) {
        e.preventDefault();
        $(this).parents('.promo-box').find('.serial').addClass('open');
    });
}

function close_promo_box() {
    $('.extensions').on('click', '.promo-box .btn-close', function (e) {
        e.preventDefault();
        $(this).parents('.promo-box').find('.serial').removeClass('open');
    });
}

function serial_activation_handler() {
    $('.extensions').on('click', '.promo-box .btn-activate-serial', function (e) {
        e.preventDefault();
        $extension = $(this).parents('.promo-box');
...
function extension_deactivate_handler() {
    $('.extensions').on('click', '.promo-box .btn-deactivate', function (e) {
        e.preventDefault();
        $extension = $(this).parents('.promo-box');
        show_extension_loader($extension);
