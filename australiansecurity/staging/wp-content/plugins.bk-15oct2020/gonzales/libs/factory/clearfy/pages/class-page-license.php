 ...
 * @since         2.0.7
 */
class Wbcr_FactoryClearfy215_LicensePage extends Wbcr_FactoryClearfy215_PageBase {
	...
	public function assets( $scripts, $styles ) {
		...
		$this->styles->add( FACTORY_CLEARFY_215_URL . '/assets/css/license-manager.css' );
		$this->scripts->add( FACTORY_CLEARFY_215_URL . '/assets/js/license-manager.js' );
	}
	...
	public function ajax_handler() {
		wbcr_factory_clearfy_215_check_license( $this->plugin );
	}
	...
	public function showPageContent() {
        ...
        <div id="wcl-license-wrapper"
             data-loader="<?php echo FACTORY_CLEARFY_215_URL . '/assets/img/loader.gif'; ?>"
	...
	public function show_license_form( $notice = false ) {
        ...
        <div id="license-manager"
             class="factory-bootstrap-424 onp-page-wrap <?= $this->get_license_type() ?>-license-manager-content">
            <div>
                <h3><?php printf( __( 'Activate %s', 'wbcr_factory_clearfy_215' ), $this->plan_name ) ?></h3>
			...
			<?php if ( is_wp_error( $notice ) ) : ?>
                <div class="license-message <?= $this->get_license_type() ?>-license-message">
                    <div class="alert <?php echo esc_attr( $notice->get_error_code() ); ?>">
                        <h4 class="alert-heading"><?php _e( $notice->get_error_message(), 'wbcr_factory_clearfy_215' ) ?></h4>
                    </div>
                </div>
			<?php endif; ?>
            <div class="onp-container">
                <div class="license-details">
					<?php if ( $this->get_license_type() == 'free' ): ?>
                        <a href="<?php echo $this->plugin->get_support()->get_pricing_url( true, 'license_page' ); ?>"
                           class="purchase-premium" target="_blank" rel="noopener">
                            ...
                            <?php _e( 'Upgrade to Premium', 'wbcr_factory_clearfy_215' ) ?>
                            </span>
                        </a>
                        <p><?php printf( __( 'Your current license for %1$s:', 'wbcr_factory_clearfy_215' ), $this->plugin->getPluginTitle() ) ?></p>
					<?php endif; ?>
                    <div class="license-details-block <?= $this->get_license_type() ?>-details-block">
                        ...
                        <div class="license-key-description">
                            <p><?php _e( 'Public License is a GPLv2 compatible license allowing you to change and use this version of the plugin for free. Please keep in mind this license covers only free edition of the plugin. Premium versions are distributed with other type of a license.', 'wbcr_factory_clearfy_215' ) ?>
                            </p>
							<?php if ( $this->is_premium && $this->premium_has_subscription ): ?>
                                <p class="activate-trial-hint">
									<?php _e( 'You use a paid subscription for the plugin updates. In case you don’t want to receive paid updates, please, click <a data-action="unsubscribe" class="wcl-control-btn" href="#">cancel subscription</a>', 'wbcr_factory_clearfy_215' ) ?>
                                </p>
							<?php endif; ?>

							<?php if ( $this->get_license_type() == 'trial' ): ?>
                                <p class="activate-error-hint">
									<?php printf( __( 'Your license has expired, please extend the license to get updates and support.', 'wbcr_factory_clearfy_215' ), '' ) ?>
                                </p>
							<?php endif; ?>
                        </div>
                        <table class="license-params" colspacing="0" colpadding="0">
                            <tr>
                                <!--<td class="license-param license-param-domain">
										...
										<span class="license-value-name"><?php _e( 'domain', 'wbcr_factory_clearfy_215' ) ?></span>
									</td>-->
                                <td class="license-param license-param-days">
                                    <span class="license-value"><?= $this->get_plan() ?></span>
                                    <span class="license-value-name"><?php _e( 'plan', 'wbcr_factory_clearfy_215' ) ?></span>
                                </td>
								<?php if ( $this->is_premium ) : ?>
                                    <td class="license-param license-param-sites">
                                        <span class="license-value">
                                            <?php echo esc_attr( $this->premium_license->get_count_active_sites() ); ?>
                                            <?php _e( 'of', 'wbcr_factory_clearfy_215' ) ?>
                                            <?php echo esc_attr( $this->premium_license->get_sites_quota() ); ?></span>
                                        <span class="license-value-name"><?php _e( 'active sites', 'wbcr_factory_clearfy_215' ) ?></span>
                                    </td>
								<?php endif; ?>
								...
								<?php if ( $this->is_premium ): ?>
                                    <td class="license-param license-param-days">
										<?php if ( $this->get_license_type() == 'trial' ): ?>
                                            <span class="license-value"><?php _e( 'EXPIRED!', 'wbcr_factory_clearfy_215' ) ?></span>
                                            <span class="license-value-name"><?php _e( 'please update the key', 'wbcr_factory_clearfy_215' ) ?></span>
										<?php else: ?>
                                            <span class="license-value">
													<?php
													...
													?>
                                                <small> <?php _e( 'day(s)', 'wbcr_factory_clearfy_215' ) ?></small>
                                             </span>
                                            <span class="license-value-name"><?php _e( 'remained', 'wbcr_factory_clearfy_215' ) ?></span>
										<?php endif; ?>
                                    </td>
								<?php endif; ?>
                            </tr>
                ...
                <div class="license-input">
                    <form action="" method="post">
						<?php if ( $this->is_premium ): ?>
                    <p><?php _e( 'Have a key to activate the premium version? Paste it here:', 'wbcr_factory_clearfy_215' ) ?><p>
					<?php else: ?>
                        <p><?php _e( 'Have a key to activate the plugin? Paste it here:', 'wbcr_factory_clearfy_215' ) ?>
                        <p>
							<?php endif; ?>
                            <button data-action="activate" class="btn btn-default wcl-control-btn" type="button" id="license-submit">
								<?php _e( 'Submit Key', 'wbcr_factory_clearfy_215' ) ?>
                            </button>
						...
						<?php if ( $this->is_premium ): ?>
                            <p style="margin-top: 10px;">
								<?php printf( __( '<a href="%s" target="_blank" rel="noopener">Lean more</a> about the premium version and get the license key to activate it now!', 'wbcr_factory_clearfy_215' ), $this->plugin->get_support()->get_pricing_url( true, 'license_page' ) ); ?>
                            </p>
						<?php else: ?>
                            <p style="margin-top: 10px;">
								<?php printf( __( 'Can’t find your key? Go to <a href="%s" target="_blank" rel="noopener">this page</a> and login using the e-mail address associated with your purchase.', 'wbcr_factory_clearfy_215' ), $this->plugin->get_support()->get_contacts_url( true, 'license_page' ) ) ?>
                            </p>
						<?php endif; ?>
                    </form>
                </div>
