...
// Don't resize images
function aven_zozo_import_filter_image_sizes( $sizes ) {
	return array();
}
 
/* Credentials Class */
class Aven_WP_Import { //Aven_WP_FileSystem_Credentials
	...
	public function __construct() {
		...
		self::aven_zozo_demo_files_download();
		
		self::aven_custom_sidebars();
		
		self::aven_widgets_import();
		
		self::aven_theme_option_import();
		
		self::aven_rev_slider_import();
		
		self::aven_theme_xml_import();
		
		self::aven_import_final_settings();		
		
	}
	
	public static function check_credentials() {
		// Get user credentials for WP filesystem API
		$page_url = wp_nonce_url( 'admin.php?page=aven-demos', 'aven-demos' );
		if ( false === ( $creds = request_filesystem_credentials( $page_url, '', false, false, null ) ) ) {
			return new WP_Error( 'XML_parse_error', esc_html__( 'There was an error when reading this WXR file', 'aven' ) );
		}
	...
	public static function aven_get_server_files($url){
	...
	public static function aven_zozo_demo_files_download() {
		...
		$url      = "http://themes.zozothemes.com/extras/importer/aven/". $demo_type ."/themewithwoo.xml";
		$theme_xml_content    = self::aven_get_server_files($url);
		
		$theme_option_file = ZOZOINCLUDES .'plugins/importer/data/theme-options.json';				
		$url      = "http://themes.zozothemes.com/extras/importer/aven/". $demo_type ."/theme-options.json";
		$theme_option_content    = self::aven_get_server_files($url);
		
		$sidebar_file = ZOZOINCLUDES .'plugins/importer/data/custom-sidebars.json';				
		$url      = "http://themes.zozothemes.com/extras/importer/aven/". $demo_type ."/custom-sidebars.json";
		$sidebar_content    = self::aven_get_server_files($url);
		
		$widget_file = ZOZOINCLUDES .'plugins/importer/data/widgets.json';				
		$url      = "http://themes.zozothemes.com/extras/importer/aven/". $demo_type ."/widgets.json";
		$widget_content    = self::aven_get_server_files($url);
		
		$rev_count = '';
		if( isset( $_POST['rev_slider'] ) && $_POST['rev_slider'] == 'yes' ){
			if( class_exists( 'RevSlider' ) ) {
				$rev_count = sanitize_text_field( $_POST['rev_slider_count'] );
			}
		}
		...
		if($access_type === 'direct')
		{
			...
			if( $rev_count ){
				...
				for( $i = 1; $i <= absint( $rev_count ); $i++ ){
					$sliders_from[$i] = "http://themes.zozothemes.com/extras/importer/aven/". $demo_type ."/rev_slider_". $i .".zip";
					$sliders[$i] = ZOZOINCLUDES .'plugins/importer/data/slider-'. $demo_type .'-'. $i .'.zip';
					//copy($sliders_from[$i], $sliders[$i]);
					$sliders_content = self::aven_get_server_files( $sliders_from[$i] );
					$wp_filesystem->put_contents( $sliders[$i], $sliders_content, FS_CHMOD_FILE );
					
				}
			}
			set_theme_mod('aven_installed_demo_id', esc_attr( $demo_type ));
			set_theme_mod('aven_demo_installed', 1);
		}	
		else
		{
			/* don't have direct write access. Prompt user with our notice */
			echo esc_html__( 'File access permission problem.', 'aven' );
		}
	}

	public static function aven_rev_slider_import(){
		if( isset( $_POST['rev_slider'] ) || $_POST['rev_slider'] == 'yes' ){
	...
	public static function aven_theme_option_import(){
		...
		if($wp_filesystem->exists($filename)){
			...
			aven_zozo_save_theme_options();
		}else{
			echo esc_html__( 'Failure to import theme options.', 'aven' );
		}
		
	}

	/* Custom Sidebars Import */
	public static function aven_custom_sidebars(){
		...
		if($wp_filesystem->exists($filename)){
			...
			if( class_exists( "aven_sidebar_generator" ) ){
				aven_sidebar_generator::init();
			}
		}else{
			echo esc_html__( 'Failure to import custom sidebars.', 'aven' );
		}
		
	}

	/* Widgets Import */
	public static function aven_widgets_import(){
		...
		if($wp_filesystem->exists($filename)){
			...
			$res = self::aven_widgets_import_process($widgets_json);
		}else{
			echo esc_html__( 'Failure to import theme options.', 'aven' );
		}
	}

	public static function aven_get_available_widgets() {
	...
	public static function aven_widgets_import_process( $data ) {
		global $wp_registered_sidebars;
		// Get all available widgets site supports
		$available_widgets = self::aven_get_available_widgets();
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
				...
				$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'aven' );
			}
			// Result for sidebar
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
					...
					$widget_message = esc_html__( 'Site does not support widget', 'aven' ); // explain why widget not imported
				}
				...
				if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message = esc_html__( 'Widget already exists', 'aven' ); // explain why widget not imported
							break;
						}
					}
				}
				...
				if ( ! $fail ) {
					...
					if ( $sidebar_available ) {
						...
						$widget_message = esc_html__( 'success', 'aven' );
					} else {
						$widget_message_type = 'warning';
						$widget_message = esc_html__( 'Imported to Inactive', 'aven' );
					}
				}
				// Result for widget instance
				$results[$sidebar_id]['widgets'][$widget_instance_id]['name'] = isset( $available_widgets[$id_base]['name'] ) ? $available_widgets[$id_base]['name'] : $id_base; // widget name or ID if name not available (not supported by site)
				$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = ! empty( $widget->title ) ? $widget->title : esc_html__( 'No Title', 'aven' ); // show "No Title" if widget instance is untitled
	...
	public static function aven_theme_xml_import(){
		...
		if($importer_error){
			...
			echo esc_html__( 'Error on import', 'aven' );
		} else {
			if ( !class_exists( 'WP_Import' ) ) {
				echo esc_html__("WP_Import Problem", "aven");
			}else{
				add_filter( 'intermediate_image_sizes_advanced', 'aven_zozo_import_filter_image_sizes', 10, 1 );
	...
	public static function aven_import_final_settings(){
