...
class AvenZozoItemRatings {
	...
	*/
	...
	public function enqueue_admin_scripts() {
		...
		wp_register_script( 'aven-zozo-rate-it', $js_inc_path . 'rate-it/jquery.rateit.min.js', array('jquery'), '1.0.22' );
		wp_enqueue_script( 'aven-zozo-rate-it' );
		
		//Enqueue this plugin's script
		wp_register_script( 'aven-zozo-item-ratings', $js_inc_path . 'rate-it/zozo-ratings-script.js', array('aven-zozo-rate-it'), '1.0' );
		wp_enqueue_script( 'aven-zozo-item-ratings' );
		
		//Enqueue stylesheet for rate it plugin - can be de-enqueued and replaced by themes :)
		wp_enqueue_style( 'aven-zozo-item-ratings-stars', $css_inc_path . 'rateit-admin.css', array(), '1.0' );
	...
	*/
	private function localize_script() {
		...
		//Init vars
		$object 	= 'AvenZozoItemRatingsVars';
		$js_vars	= array();
		...
		wp_localize_script( 'aven-zozo-item-ratings', $object, $js_vars );
	}
	...
	*/
	public function add_custom_column_content( $config_option_key, $column_name, $post_ID ) {
		...
		if ($column_name == $column_slug) {  
	        ...
	        //Echo out content
	        echo apply_filters( 'aven_zozo_item_ratings_column_stars', $output, $column_name, $option, $post_ID );
	    }
	...
	public function add_custom_meta_box( $config_option_key ) {
		...
		foreach( self::$class_config as $option ) {
			...
			if( $option['meta_key'] === $config_option_key ) {
		        ...
		        //Echo out content
		        echo apply_filters( 'aven_zozo_item_ratings_metabox_stars', $output, $option, $post_ID );
