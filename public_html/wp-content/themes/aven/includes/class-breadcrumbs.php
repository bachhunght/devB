...
if ( ! class_exists( 'Aven_Zozo_Breadcrumbs' ) ) {
	class Aven_Zozo_Breadcrumbs {
		...
		public function __construct() {
			...
			$defaults = array(
				 ...
				 'show_terms'  				=> 1,
				 'home_label'      			=> esc_html__( 'Home', 'aven' ),
				 'tag_archive_prefix'   	=> esc_html__( 'Tag:', 'aven' ),
				 'search_prefix'        	=> esc_html__( 'Search:', 'aven' ),
				 'error_prefix' 			=> esc_html__( '404 - Page not Found', 'aven' ),
			);
			
			// Setup a filter for changeable variables and meger it with the defaults
			$args 		= apply_filters( 'aven_zozo_breadcrumbs_defaults', $defaults );
		 ...
		 * @return void
		 */				
		public function aven_zozo_display_breadcrumbs() {
			...
			if ( function_exists('yoast_breadcrumb') && $options && $options['breadcrumbs-enable'] === true ) {
				...
				$this->aven_zozo_display_theme_breadcrumbs();
			}
		 ...
		 * @return void
		 */				
		private function aven_zozo_display_theme_breadcrumbs() {
					
			// Add the "Home" link
			$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_home();
			...
			if ( class_exists( 'Woocommerce' ) && ( ( is_woocommerce() && is_archive() && ! is_shop() ) || is_cart() || is_checkout() || is_account_page() ) ) {
				$this->breadcrumb_markup .= $this->aven_zozo_get_woocommerce_shop_page();
			}
			
			// bbPress path prefix (e.g "Forums" )
			if ( class_exists( 'bbPress' ) && is_bbpress() && ( bbp_is_topic_archive() || bbp_is_single_user() || bbp_is_search() ) ) {
				$this->breadcrumb_markup .= $this->aven_zozo_get_bbpress_main_archive_page();
			}
			...
			if ( is_singular() ) {
				...
				if ( isset( $this->post->post_type ) && get_post_type_archive_link( $this->post->post_type ) &&	$this->show_post_type_archive ) {
					$this->breadcrumb_markup .= $this->aven_zozo_get_post_type_archive();
				}
				
				// If the post doesn't have parents
				if ( isset( $this->post->post_parent ) && $this->post->post_parent == 0 ) {
					$this->breadcrumb_markup .= $this->aven_zozo_get_post_terms();
				// If there are parents; mostly for pages
				} else {
					$this->breadcrumb_markup .= $this->aven_zozo_get_post_ancestors();
				}
				
				$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup();
			} else {
				// Custom post types archives
				if ( is_post_type_archive() ) {
					$this->breadcrumb_markup .= $this->aven_zozo_get_post_type_archive( FALSE );

					// Search on custom post type (e.g. Woocommerce)
					if ( is_search() ) {
						$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( 'search' );
					}
				// Taxonomy Archives
					...
					$this->breadcrumb_markup .= $this->aven_zozo_get_taxonomies();
					$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( 'term' );
				// Date Archives
				} elseif ( is_date() ) {
					global $wp_locale;
					// Set variables
					...
					if ( is_year() ) {
						$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( 'year' );
					// Month Archive, needs year link and month leaf
					} elseif ( is_month() ) {
						$this->breadcrumb_markup .= $this->aven_zozo_get_single_breadcrumb_markup( $year, get_year_link( $year ) );
						$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( 'month' );
					// Day Archive, needs year and month link and day leaf
					} elseif ( is_day() ) {
						$this->breadcrumb_markup .= $this->aven_zozo_get_single_breadcrumb_markup( $year, get_year_link( $year ) );
						$this->breadcrumb_markup .= $this->aven_zozo_get_single_breadcrumb_markup( $month_name, get_month_link( $year, $month ) );
						$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( 'day' );						
					}			
				// Author Archives
				} elseif ( is_author() ) {
					$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( 'author' );
				// Search Page
				} elseif ( is_search() ) {
					$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( 'search' );
				// 404 Page
					...
					if ( class_exists( 'TribeEvents' ) && ( tribe_is_event() || aven_is_events_archive() ) ) { 
						$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( 'events' );
					// Default case
					} else {
						$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( '404' );
					}
				// bbPress
				} elseif ( class_exists( 'bbPress' ) ) {
				 	// Search Page
				 	if ( bbp_is_search() ) {
				 		$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( 'bbpress_search' );
				 	// User page
				 	} elseif ( bbp_is_single_user() ) {
						$this->breadcrumb_markup .= $this->aven_zozo_get_breadcrumb_leaf_markup( 'bbpress_user' );
					}
				} 
			}
		}
		 ...
		 */			
		private function aven_zozo_get_breadcrumb_home() {
			$home_link = '';
		
			// If the home page is a real page
			if ( ! is_front_page() ) {
				$home_link = $this->aven_zozo_get_single_breadcrumb_markup( $this->home_label, get_home_url() );
			// If the home page is the main blog page
			} elseif ( is_home() ) { 
				$blog_title = '';
				$blog_title = aven_zozo_get_theme_option( 'zozo_blog_title' );
				
				if( isset( $blog_title ) && $blog_title == '' ) {
					$blog_title = esc_html__( 'Home', 'aven' );
				}
				$home_link = $this->aven_zozo_get_single_breadcrumb_markup( $blog_title );
			}
		 ...
		 */			
		private function aven_zozo_get_post_terms() {
			$terms_markup = '';
			...
			if ( count( $terms ) == 1 ) {
				...
				if ( $term_parent ) {
					...
					foreach ( $term_tree as $term_id ) {
						...
						$terms_markup .= $this->aven_zozo_get_single_breadcrumb_markup( $term_object->name, get_term_link( $term_object ) );
					}
				// We have a single term, so put it out
				} else {
					$terms_markup = $this->aven_zozo_get_single_breadcrumb_markup( $terms[0]->name, get_term_link( $terms[0] ) );
				}
			// The post has multiple terms
			} else {
				// The lexicographically smallest term will be part of the breadcrump rich snippet path
				$terms_markup = $this->aven_zozo_get_single_breadcrumb_markup( $terms[0]->name, get_term_link( $terms[0] ), FALSE );
				...
				foreach ( $terms as $term ) {
					...
					if ( ++$i == $max_index ) {
						$terms_markup .= ', '  . $this->aven_zozo_get_single_breadcrumb_markup( $term->name, get_term_link( $term ), TRUE, FALSE );
					} else {
						$terms_markup .= ', '  . $this->aven_zozo_get_single_breadcrumb_markup( $term->name, get_term_link( $term ), FALSE, FALSE );
					}
				}
			}
		 ...
		 */			
		private function aven_zozo_get_post_ancestors() {
			$ancestors_markup = '';
			...
			foreach ( $post_ancestor_ids as $post_ancestor_id ) {
				...
				$ancestors_markup .= $this->aven_zozo_get_single_breadcrumb_markup( $post_ancestor->post_title, get_permalink( $post_ancestor->ID ) );
			}
		 ...
		 */
		private function aven_zozo_get_taxonomies() {
			global $wp_query;
			...
			if ( $term->parent != 0 &&
				...
				foreach ( $term_ancestors as $term_ancestor ) {
					...
					$terms_markup .= $this->aven_zozo_get_single_breadcrumb_markup( $term_object->name, get_term_link( $term_object->term_id, $term->taxonomy ) );
				}
			}
		 ...
		 */			
		private function aven_zozo_get_post_type_archive( $linked = TRUE ) {
			global $wp_query;
			...
			if ( is_object( $post_type_object ) ) {
				...
				if ( $post_type == 'product' && $woocommerce_shop_page = $this->aven_zozo_get_woocommerce_shop_page( $linked ) ) {
				...
				if ( class_exists( 'bbPress' ) && $post_type == 'topic' ) {
					...
					return $this->aven_zozo_get_single_breadcrumb_markup( $archive_title, $link );
				}
		 ...
		 */				
		private function aven_zozo_get_woocommerce_shop_page( $linked = TRUE ) {
			global $wp_query;
			...
			if ( is_object( $post_type_object ) && class_exists( 'Woocommerce' ) && ( is_woocommerce() || is_cart() || is_checkout() || is_account_page() )	) {
				...
				$shop_page_markup = $this->aven_zozo_get_single_breadcrumb_markup( $shop_page_name, $link );
			}
		 ...
		 */				
		private function aven_zozo_get_bbpress_main_archive_page() {
			global $wp_query;
		
			return $this->aven_zozo_get_single_breadcrumb_markup( bbp_get_forum_archive_title(), get_post_type_archive_link( 'forum' ) );
		}
		 ...
		 */	
		private function aven_zozo_get_breadcrumb_leaf_markup( $object_type = '' ) {
		 ...
		 */
		private function aven_zozo_get_single_breadcrumb_markup( $title, $link = '', $separator = TRUE, $microdata = TRUE ) {
