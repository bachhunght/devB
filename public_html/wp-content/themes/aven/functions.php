...
define( 'ZOZO_REDUX', get_template_directory() . '/framework/admin/ReduxCore' );
...
function aven_zozo_check_registered_post_types() {
...
add_action( 'init', 'aven_zozo_check_registered_post_types' );
...
aven_zozo_item_ratings_init();
function aven_zozo_item_ratings_init() {
	...
	new AvenZozoItemRatings( $config_options );
}
...
/* Demo Import AJAX */
if( ! function_exists('aven_demo_content_importer') ) {
    function aven_demo_content_importer() {
		
		if( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'aven_demo_import_@#$%^&*(' ) ) {
			echo "!security issue";
			wp_die(); 
		}
    ...
    add_action('wp_ajax_zozo_import_demo_data', 'aven_demo_content_importer');
}
