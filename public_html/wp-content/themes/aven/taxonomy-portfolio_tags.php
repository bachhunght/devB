			...
			<div class="zozo-row row">	
				<div id="primary" class="content-area <?php aven_zozo_primary_content_classes(); ?>">
					<div id="content" class="site-content">
						...
						if( aven_zozo_get_theme_option( 'zozo_portfolio_archive_count' ) != '' ) {
							$posts = aven_zozo_get_theme_option( 'zozo_portfolio_archive_count' );
						} else {
							$posts = 10;
						}
						
						if( aven_zozo_get_theme_option( 'zozo_portfolio_archive_layout' ) != '' ) {
							$style = aven_zozo_get_theme_option( 'zozo_portfolio_archive_layout' );
						} else {
							$style = 'grid';
						}
						
						if( aven_zozo_get_theme_option( 'zozo_portfolio_archive_columns' ) != '' ) {
							$grid_columns = aven_zozo_get_theme_option( 'zozo_portfolio_archive_columns' );
						} else {
							$grid_columns = 4;
						}
						...
						if( aven_zozo_get_theme_option( 'zozo_portfolio_archive_gutter' ) != '' ) {
							$grid_gutter = aven_zozo_get_theme_option( 'zozo_portfolio_archive_gutter' );
						} else {
							$grid_gutter = 30;
						}
						...
						<?php if ( $query->have_posts() ):
							echo '<div class="zozo-portfolio-grid-wrapper zozo-isotope-grid-system clearfix">';
								...
								while ( $query->have_posts() ): $query->the_post();
									...
									$media_output = '';
									$media_output = aven_zozo_portfolio_media_output( $post->ID, $style, $grid_columns, $term_id ); 
									
									$media_icon_output = '';
									$media_icon_output = aven_zozo_portfolio_media_icon_output( $post->ID, $term_id ); ?>
										...
										<div class="portfolio-content">
											...
											echo '<div class="portfolio-image-overlay-wrapper">';
												if( isset( $media_output ) && $media_output != '' ) {
													echo aven_zozo_portfolio_media_output( $post->ID, $style, $grid_columns, $term_id );
												}  else {				
											...
											if( isset( $style ) && $style == 'classic' ) {
												echo '<div class="portfolio-mask overlay-mask position-center">';
													echo '<ul class="overlay-buttons fade-up">';					
														if( isset( $media_icon_output ) && $media_icon_output != '' ) {
															echo aven_zozo_portfolio_media_icon_output( $post->ID, $term_id );
														} else {
												...
												echo '<div class="portfolio-inner-wrapper">';
													echo '<div class="portfolio-inner-content">';
														...
														echo '<p>' . aven_zozo_shortcode_stripped_excerpt( get_the_excerpt(), 15 ) . '</p>';
														
														echo '<a href="'. get_permalink() .'" class="btn btn-more" title="'.get_the_title().'">'. esc_html__('Read more', 'aven') .'</a>';
												...
												echo '<div class="portfolio-mask overlay-mask position-center">';
													echo '<div class="portfolio-title">';
														...
														if( isset( $grid_columns ) && $grid_columns == 4 ) {
															echo '<p>' . aven_zozo_shortcode_stripped_excerpt( get_the_excerpt(), 5 ) . '</p>';
														} else {
															echo '<p>' . aven_zozo_shortcode_stripped_excerpt( get_the_excerpt(), 8 ) . '</p>';
														}
													echo '</div>';
								...
								<?php echo aven_zozo_pagination( $query->max_num_pages, '' ); ?>
								</div>
