<?php
class Aven_Zozo_Mailchimp_Form_Widget extends WP_Widget {	
	...
	function widget($args, $instance)
	{
		...
		$description = !empty($instance['description']) ? $instance['description'] : esc_html__( 'Subscribe to Our Newsletter to get Important News, Amazing Offers & Inside Scoops:', 'aven' );
	...
	function form($instance)
	{
		...
		$lists = aven_zozo_get_mailing_lists_format(); ?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('description') ); ?>"><?php _e('Description:', 'aven'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('mailchimp_list') ); ?>"><?php _e( 'Choose Mailing List:', 'aven' ); ?></label>			
...
function aven_zozo_mailchimp_form_load()
{
	register_widget('Aven_Zozo_Mailchimp_Form_Widget');
}

add_action('widgets_init', 'aven_zozo_mailchimp_form_load');
?>