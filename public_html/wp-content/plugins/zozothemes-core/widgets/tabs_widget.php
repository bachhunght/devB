<?php
class Aven_Zozo_Tabs_Widget extends WP_Widget {
			...
			<div class="zozo-tabs-wrapper tabs">
				<ul id="zozo-tabs" class="nav nav-tabs" role="tablist">
					<?php if($show_popular_posts == 'on') { ?>
						<li class="active"><h6><a href="#tab-popular" data-toggle="tab"><?php _e('Popular', 'aven'); ?></a></h6></li>
					<?php } ?>
					<?php if($show_recent_posts == 'on') { ?>
						<li><h6><a href="#tab-recent" data-toggle="tab"><?php _e('Recent', 'aven'); ?></a></h6></li>
					<?php } ?>
								...
								<ul class="widget-posts-list">
									...
									<li>
										...
										<div class="widget-entry-content">
											<h6><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a></h6>
											<div class="widget-entry-meta">
												<span class="entry-date"><?php the_time( aven_zozo_get_theme_option( 'zozo_blog_date_format' ) ); ?></span>
											</div>
										</div>										
									</li>
									<?php endwhile; ?>
	...
	function form($instance)
	{
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id('show_popular_posts') ); ?>"><?php _e('Show Popular Posts', 'aven'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id('show_recent_posts') ); ?>"><?php _e('Show Recent Posts', 'aven'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id('show_comments') ); ?>"><?php _e('Show Comments', 'aven'); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('posts_count') ); ?>"><?php _e('Number of popular posts:', 'aven'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('recent_posts') ); ?>"><?php _e('Number of recent posts:', 'aven'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('comments_count') ); ?>"><?php _e('Number of comments:', 'aven'); ?></label>
...
function aven_zozo_tabs_load()
{
	register_widget('Aven_Zozo_Tabs_Widget');
}

add_action('widgets_init', 'aven_zozo_tabs_load');