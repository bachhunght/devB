				...
				<div id="primary" class="content-area <?php aven_zozo_primary_content_classes(); ?>">
					<div id="content" class="site-content">	
						<?php if ( have_posts() ):
							...
							$theme_fields 	= array('socials' 		=> aven_zozo_get_theme_option( 'zozo_blog_social_sharing' ),
													'author_info' 	=> aven_zozo_get_theme_option( 'zozo_blog_author_info' ),
													'comments' 		=> aven_zozo_get_theme_option( 'zozo_blog_comments' ),
													'related' 		=> aven_zozo_get_theme_option( 'zozo_show_related_posts' ),
													'navigation' 	=> aven_zozo_get_theme_option( 'zozo_blog_prev_next' ),
												);
							...
							if( isset( $gallery_images_type ) && ( $gallery_images_type == '' || $gallery_images_type == 'default' ) ) {
								$options_gallery_type = aven_zozo_get_theme_option( 'zozo_single_blog_carousel' );
							...
							<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
								<div class="post-inner-wrapper">
									<?php if( ! aven_zozo_get_theme_option( 'zozo_blog_post_featured_image' ) ): ?>
										<?php echo aven_zozo_output_blog_post_slider( $post->ID, 'aven-blog-large', $gallery_images_type ); ?>
									<?php endif; ?>
									...
									<?php } else { ?>
										...
										<div class="entry-meta-wrapper">
											<ul class="entry-meta">
												<!-- Entry Author -->
												<?php if( ! aven_zozo_get_theme_option( 'zozo_blog_post_meta_author' ) ): ?>
													<li class="author"><i class="simple-icon icon-user"></i><?php the_author_posts_link(); ?></li>
												<?php endif; ?>
												<!-- Entry Date -->
												<?php if( ! aven_zozo_get_theme_option( 'zozo_blog_post_meta_date' ) ): ?>
													<li class="posted-date"><i class="simple-icon icon-calendar"></i><?php the_time( aven_zozo_get_theme_option( 'zozo_blog_date_format' ) ); ?></li>
												<?php endif; ?>					
												<!-- Entry Category -->
												<?php if( ! aven_zozo_get_theme_option( 'zozo_blog_post_meta_categories' ) ): ?>
													<li class="category"><i class="simple-icon icon-folder"></i><?php echo get_the_category_list(', '); ?></li>
												<?php endif; ?>
												<!-- Comments -->
												<?php if( ! aven_zozo_get_theme_option( 'zozo_blog_post_meta_comments' ) ): ?>
													<?php if ( comments_open() ) { ?>
														<li class="comments-link"><i class="simple-icon icon-bubbles"></i><?php comments_popup_link( '<span class="leave-reply">' . esc_html__( '0 Comments', 'aven' ) . '</span>', esc_html__( '1 Comments', 'aven' ), esc_html__( '% Comments', 'aven' ) ); ?>
													</li>
													<?php }
												endif; ?>
											</ul>
											...
											<?php if( has_tag() ) { ?>
												...
												<div class="col-md-6">
													<?php if( $post_values['socials'] == 'show' || $post_values['socials'] == 1 ) {
														...
														aven_zozo_display_social_sharing_icons();
														echo '</div>';
													} ?>
												</div>
											<?php } else { ?>
