...
if ( ! function_exists( 'aven_zozo_vc_events_list_shortcode' ) ) {
	function aven_zozo_vc_events_list_shortcode( $atts, $content = NULL ) {
		...
		if( $events_query ) {
			$output .= '<div id="zozo_events_'.$events_id.'" class="'. esc_attr( $main_classes ) .'">';
								...
								switch ( trim( $content_order ) ) {
									...
									case 'thumbnail' :
										if( isset( $thumb_image ) && $thumb_image == "true" ) {
											...
											if( ! empty($thumbWidth) && ! empty($thumbHeight) ) {
												...
												$size = ( !empty($thumbWidth) && !empty($thumbHeight) ) ? array( $thumbWidth, $thumbHeight ) : 'aven-theme-mid';
									...
									case 'excerpt' :
										if( isset( $excerpt ) && $excerpt == "true" ) {
											...
											$output .= '<p class="ecs-excerpt"'. $content_styles .'>' . aven_zozo_shortcode_stripped_excerpt( get_the_content(), $excerptLength ) . '</p>';
										}
										break;
				...
				if( isset( $viewall ) && $viewall == "yes" ) {
					$output .= '<span class="ecs-all-events"><a href="' . apply_filters( 'ecs_event_list_viewall_link', tribe_get_events_link(), $atts ) .'" class="btn btn-default" rel="bookmark">' . translate( 'View All Events', 'aven' ) . '</a></span>';
				}
			$output .= '</div>';
		} else { //No Events were Found
			$output .= translate( $message, 'aven' );
		}
...
if ( ! function_exists( 'aven_zozo_vc_events_list_shortcode_map' ) ) {
	function aven_zozo_vc_events_list_shortcode_map() {
		
		vc_map( 
			array(
				"name"					=> esc_html__( "Events List", "aven" ),
				"description"			=> esc_html__( "List your events from The Events Calendar Plugin.", 'aven' ),
				...
				"params"				=> array(					
					...
					array(
						'type' 			=> 'autocomplete',
						'heading' 		=> esc_html__( 'Include Events', 'aven' ),
						'param_name' 	=> 'include_events',
						"admin_label" 	=> true,
						'description' 	=> esc_html__( 'Add events by title.', 'aven' ),
						'settings' 		=> array(
							'multiple' 	=> true,
							'sortable' 	=> true,
						),
					...
					array(
						'type' 			=> 'autocomplete',
						'heading' 		=> esc_html__( 'Exclude Events', 'aven' ),
						'param_name' 	=> 'exclude_events',
						"admin_label" 	=> true,
						'description' 	=> esc_html__( 'Remove events by title.', 'aven' ),
						'settings' 		=> array(
							'multiple' 	=> true,
							'sortable' 	=> true,
						),
					...
					array(
						"type"			=> 'dropdown',
						"heading"		=> esc_html__( "Order by Key", "aven" ),
						"param_name"	=> "orderkey",
						"value"			=> array(
							esc_html__( "Start Date", "aven" )	=> "startdate",
							esc_html__( "End Date", "aven" )		=> "enddate" ),
					),
					array(
						"type"			=> 'dropdown',
						"heading"		=> esc_html__( "Event Title Type", "aven" ),
					...
					array(
						"type"			=> "dropdown",
						"heading"		=> esc_html__( "Event Title Text Transform", 'aven' ),
					...
					array(
						"type"			=> "textfield",
						"heading"		=> esc_html__( "Event Title Font Size", "aven" ),
						"param_name"	=> "title_size",
						"description" 	=> esc_html__( "Enter Title Font Size in px. Ex: 18px", "aven" ),
					),
					array(
						"type"			=> 'dropdown',
						"heading"		=> esc_html__( "Show Event Details ?", "aven" ),
					...
					array(
						"type"			=> 'dropdown',
						"heading"		=> esc_html__( "Show Event Venue ?", "aven" ),
						"param_name"	=> "venue",	
						"value"			=> array(
							esc_html__( "Yes", "aven" )		=> "true",
							esc_html__( "No", "aven" )		=> "false" ),
					...
					array(
						"type"			=> 'dropdown',
						"heading"		=> esc_html__( "Show Thumbnail Image ?", "aven" ),
					...
					array(
						"type"			=> 'textfield',
						"heading"		=> esc_html__( "Thumbnail Width", "aven" ),
					...
					array(
						"type"			=> 'textfield',
						"heading"		=> esc_html__( "Thumbnail Height", "aven" ),
					...
					array(
						"type"			=> 'textfield',
						"heading"		=> esc_html__( "Message", "aven" ),
						"param_name"	=> "message",	
						"value"			=> "",
						'description'	=> esc_html__('Message to show when there are no events.','aven'),
					),
					array(
						"type"			=> 'dropdown',
						"heading"		=> esc_html__( "View All Events", "aven" ),
						"param_name"	=> "viewall",	
						"value"			=> array(
							esc_html__( "Yes", "aven" )		=> "yes",
							esc_html__( "No", "aven" )		=> "no" ),
						'description'	=> esc_html__('Choose to show "View all events" or not.','aven'),
					),
					array(
						"type"			=> 'textfield',
						"heading"		=> esc_html__( "Content Order", "aven" ),
						"param_name"	=> "contentorder",	
						"value"			=> "title, thumbnail, excerpt, date, venue",
						'description'	=> esc_html__('Manage the order of content with commas.','aven'),
					),
					array(
						"type"			=> 'textfield',
						"heading"		=> esc_html__( "Events from Specific Month", "aven" ),
						"param_name"	=> "month",	
						"value"			=> "",
						'description'	=> esc_html__('Type "current" for displaying current month only. Ex: 2015-06 or current','aven'),
					),
					array(
						"type"			=> 'dropdown',
						"heading"		=> esc_html__( "Show Outdated Events", "aven" ),
						"param_name"	=> "past",
						"value"			=> array(
							esc_html__( "No", "aven" )		=> "no",
							esc_html__( "Yes", "aven" )		=> "yes" ),
					...
					array(
						...
						"value"			=> "",
						'description'	=> esc_html__('Enter border width. Ex: 1px or 2px','aven'),
						"group"			=> esc_html__( "Design", "aven" ),
					),
					array(
						"type"			=> 'colorpicker',
						"heading"		=> esc_html__( "Border Color", "aven" ),
						"param_name"	=> "border_color",
						"group"			=> esc_html__( "Design", "aven" ),
					),
					array(
						"type"			=> 'colorpicker',
						"heading"		=> esc_html__( "Event Title Color", "aven" ),
...
add_action( 'vc_before_init', 'aven_zozo_vc_events_list_shortcode_map' );