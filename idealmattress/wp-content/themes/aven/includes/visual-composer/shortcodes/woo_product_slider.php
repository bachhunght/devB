...
if ( ! function_exists( 'aven_zozo_vc_woo_product_slider_shortcode' ) ) {
	function aven_zozo_vc_woo_product_slider_shortcode( $atts, $content = NULL ) {
		...
		if( $latest_product_query->have_posts() ) {
			...
			$output .= '<div id="zozo-woo-latest-slider-' . $product_slider_id. '" class="zozo-owl-carousel zozo-woo-latest-slider owl-carousel"' . $data_attr . '>';
				...
				while($latest_product_query->have_posts()) : $latest_product_query->the_post();
					global $product;
					$woo_product_img 	= wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), 'aven-theme-mid' );
					...
					$output .= '<div class="woo-latest-slider-item">';
						...
						$output .= '<div class="woo-latest-product-box">';
							...
							$output .= '<div class="product-buttons-wrapper"><div class="product-buttons">';
								$output .= '<a href="' . get_permalink() . '" class="woo-show-details">' . esc_html__( 'Show Details', 'aven' ) . '</a>';
							$output .= '</div></div>';
							$output .= '</div>';
						$output .= '</div>';
...
if ( ! function_exists( 'aven_zozo_vc_woo_product_slider_shortcode_map' ) ) {
	function aven_zozo_vc_woo_product_slider_shortcode_map() {
				
		vc_map( 
			array(
				"name"					=> esc_html__( "Woo Latest Product Slider", "aven" ),
				"description"			=> esc_html__( "Show latest products in Slider.", 'aven' ),
				...
				"params"				=> array(
					...
					array(
						"type"			=> "textfield",
						"heading"		=> esc_html__( "Number Of Products", "aven" ),
						"param_name"	=> "products",
						'admin_label' 	=> true,
						'value' 		=> '',
					),
...
add_action( 'vc_before_init', 'aven_zozo_vc_woo_product_slider_shortcode_map' );