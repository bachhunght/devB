...
if( isset( $portfolio_details ) && $portfolio_details == 'yes' ) {
	...
	if( isset( $portfolio_btn_text ) && $portfolio_btn_text == '' ) {
		$portfolio_btn_text = esc_html__( 'View Project', 'aven' );
	} 

	$related_slider 		= get_post_meta( $post->ID, 'zozo_portfolio_related_slider', true );
	if( isset( $related_slider ) && $related_slider == '' ) {
		$related_slider_opt = aven_zozo_get_theme_option( 'zozo_portfolio_related_slider' );
	...
	$cat_label 				= aven_zozo_get_theme_option( 'zozo_portfolio_category_label' );
	$tag_label 				= aven_zozo_get_theme_option( 'zozo_portfolio_tag_label' );
	$client_label			= aven_zozo_get_theme_option( 'zozo_portfolio_client_label' );
	$date_label				= aven_zozo_get_theme_option( 'zozo_portfolio_date_label' );
	$est_label				= aven_zozo_get_theme_option( 'zozo_portfolio_estimation_label' );
	$dur_label				= aven_zozo_get_theme_option( 'zozo_portfolio_duration_label' );
} ?>
						...
						<?php if ( have_posts() ):
							...
							if( isset( $media_type ) && $media_type != 'none' ) {
								...
								if( isset( $media_type ) && $media_type == 'carousel' ) {
									$media_content .= aven_zozo_portfolio_media_output( $post->ID, 'single', '', $post->ID );
								} 
								// Audio Only
								else if( isset( $media_type ) && $media_type == 'audio' ) {
								...
								// Featured Image
									...
									$portfolio_full_img = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'aven-portfolio-single' );
								...
								// Stack Images
									...
									foreach( $media_images as $image ) {
										$post_image = aven_zozo_get_portfolio_post_media( '', '', $image, 'aven-portfolio-single', '1300', '500');
										
										$media_content .= '<div class="portfolio-img"><img class="img-responsive" src="' . $post_image[0] . '" width="' . $post_image[1] . '" height="' . $post_image[2] . '" alt="' . $post_image[3] . '" /></div>';
									...
									$media_content .= '<div class="zozo-isotope-wrapper portfolio-isotope-wrapper">';
										...
										foreach( $media_images as $image ) {
											if( isset( $isotope_type ) && $isotope_type == 'masonry' ) {
												$post_image = aven_zozo_get_portfolio_post_media( '500', '', $image );
											} else {
												$post_image = aven_zozo_get_portfolio_post_media( '', '', $image, 'aven-portfolio-mid', '560', '385');
											}
							...
							if( isset( $portfolio_details ) && $portfolio_details == 'yes' ) {
								...
								if( isset( $portfolio_custom_text ) && $portfolio_custom_text != '' ) {
									...
									$details_content .= '<h6 class="portfolio-custom-text">'. do_shortcode( $portfolio_custom_text ) .'</h6>';
									$details_content .= '</div>';
								}
								...
								if( isset( $portfolio_share ) && $portfolio_share == 1 ) {
									...
									$details_content .= aven_zozo_display_social_sharing_icons( 'no' );
									$details_content .= '</div>';
								} 
							...
							if( isset( $related_slider ) && $related_slider != 1 ) {
								aven_zozo_related_portfolio_slider();
							}
							
							if( aven_zozo_get_theme_option( 'zozo_portfolio_prev_next' ) == 1 ) {
								aven_zozo_postnavigation();
							}
