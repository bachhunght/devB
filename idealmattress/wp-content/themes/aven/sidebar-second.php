...
$sidebar_widget = $choosed_layout = $layouts = $home_id = '';
	...
	if( is_archive() ) {
		$choosed_layout = aven_zozo_get_theme_option( 'zozo_blog_archive_layout' );
	}
	...
	if( is_home() ) {
		...
		if( !$choosed_layout ) {
			$choosed_layout = aven_zozo_get_theme_option( 'zozo_blog_layout' );
		}
	...
	if ( is_singular( 'post' ) ) {
		$choosed_layout = get_post_meta( $post->ID, 'zozo_layout', true );
		if( !$choosed_layout ) {
			$choosed_layout = aven_zozo_get_theme_option( 'zozo_single_post_layout' );
		}
	...
	if( !$choosed_layout ) {			
		if( aven_zozo_get_theme_option( 'zozo_layout' ) != '' ) {		
			$choosed_layout = aven_zozo_get_theme_option( 'zozo_layout' );
		}
		else {
...
?>
<div id="secondary-sidebar" class="secondary-sidebar sidebar <?php aven_zozo_secondary_sidebar_classes(); ?>">
