<?php
class Aven_Zozo_Instagram_Widget extends WP_Widget {
	...
	function scrape_instagram($user_name, $photos_count) {
		...
		if(false === ($instagram = get_transient('zozo-instagram-'.sanitize_title_with_dashes($user_name)))) {
			...
			if ( is_wp_error( $remote ) ) {
				return new WP_Error( 'site_down', esc_html__( 'Unable to communicate with Instagram.', 'aven' ) );
			}
			
			if ( 200 != wp_remote_retrieve_response_code( $remote ) ) {
				return new WP_Error( 'invalid_response', esc_html__( 'Instagram did not return a 200.', 'aven' ) );
			}
			...
			if ( ! $insta_array ) {
				return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'aven' ) );
			}
			...
			// new style
				...
				return new WP_Error( 'bad_josn_2', esc_html__( 'Instagram has returned invalid data.', 'aven' ) );
			}
			
			if ( !is_array( $images ) ) {
				return new WP_Error( 'bad_array', esc_html__( 'Instagram has returned invalid data.', 'aven' ) );
			}
			...
			switch ( $type ) {
				...
				default:
					foreach ( $images as $image ) {
						...
						$instagram[] = array(
							'description'   => esc_html__( 'Instagram Image', 'aven' ),
		...
		if ( ! empty( $instagram ) ) {
			...
			return new WP_Error( 'no_images', esc_html__( 'Instagram did not return any images.', 'aven' ) );
	...
	function form($instance)
	{
		...
		$sizes = array(
			'thumbnail' => esc_attr__( 'Thumbnail', 'aven' ),
			'medium' => esc_attr__( 'Medium', 'aven' ),
			'large' => esc_attr__( 'Large', 'aven' )			
		);
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('user_name') ); ?>"><?php _e('User name:', 'aven'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('photo_count') ); ?>"><?php _e('Number of Photos to show:', 'aven'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('size') ); ?>"><?php _e( 'Sizes:', 'aven' ); ?></label>			
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('link_text') ); ?>"><?php _e('Link Text:', 'aven'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('link_target') ); ?>"><?php _e('Link Target:', 'aven'); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id('link_target' )); ?>" name="<?php echo esc_attr( $this->get_field_name('link_target') ); ?>" class="widefat" style="width:100%;">
				<option value="_blank" <?php echo selected(esc_attr($instance['link_target']), '_blank', false); ?>><?php _e('Open in a New Tab', 'aven'); ?></option>
				<option value="_self" <?php echo selected(esc_attr($instance['link_target']), '_self', false); ?>><?php _e('Open in a Same Tab', 'aven'); ?></option>				
			</select>
		</p>
	<?php }
}

function aven_zozo_instagram_load()
{
	register_widget('Aven_Zozo_Instagram_Widget');
}

add_action('widgets_init', 'aven_zozo_instagram_load');
?>