<?php
class Aven_Zozo_Call_To_Action_Widget extends WP_Widget {
	...
	function form($instance)
	{
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id('full_width') ); ?>"><?php _e('Full Width', 'aven'); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('text') ); ?>"><?php _e('Text:', 'aven'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('sub_text') ); ?>"><?php _e('Sub Title:', 'aven'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id('show_button') ); ?>"><?php _e('Show Button', 'aven'); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('button_text') ); ?>"><?php _e('Button Text:', 'aven'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('button_link') ); ?>"><?php _e('Button Link:', 'aven'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('button_target') ); ?>"><?php _e('Button Target:', 'aven'); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id('button_target' )); ?>" name="<?php echo esc_attr( $this->get_field_name('button_target') ); ?>" class="widefat" style="width:100%;">
				<option value="_blank" <?php echo selected(esc_attr($instance['button_target']), '_blank', false); ?>><?php _e('Open in a New Tab', 'aven'); ?></option>
				<option value="_self" <?php echo selected(esc_attr($instance['button_target']), '_self', false); ?>><?php _e('Open in a Same Tab', 'aven'); ?></option>				
			</select>
		</p>
	<?php }
}

function aven_zozo_call_to_action_load()
{
	register_widget('Aven_Zozo_Call_To_Action_Widget');
}

add_action('widgets_init', 'aven_zozo_call_to_action_load');
?>