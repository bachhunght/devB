<?php
class Aven_Zozo_Flickr_Widget extends WP_Widget {
	...
	function form($instance)
	{
		...
		$sizes = array(
			's' => esc_attr__( 'Standard', 'aven' ),
			't' => esc_attr__( 'Thumbnail', 'aven' ),
			'q' => esc_attr__( 'Large Square', 'aven' ),
			'm' => esc_attr__( 'Medium', 'aven' )
		);
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('user_id') ); ?>"><?php _e('Flickr ID:', 'aven'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('api_key') ); ?>"><?php _e('API Key:', 'aven'); ?></label>
...
function aven_zozo_flickr_load()
{
	register_widget('Aven_Zozo_Flickr_Widget');
}

add_action('widgets_init', 'aven_zozo_flickr_load');
?>