...
function zozo_day_counter_shortcode( $atts, $content = null ) {
	...
	wp_enqueue_script( 'aven-zozo-countdown-plugin-js' );
	wp_enqueue_script( 'aven-zozo-countdown-js' );
...
function zozo_contact_form_shortcode( $atts, $content = null ) {
	...
	$name = esc_html__('Name', 'zozothemescore');
	$email = esc_html__('Email', 'zozothemescore');
	$phone = esc_html__('Phone', 'zozothemescore');
	$message = esc_html__('Message', 'zozothemescore');
	
	$custom_msgs = '';	
	$custom_msgs .= ' data-name_not_empty="'. esc_html__( "The name is required and cannot be empty", "zozothemescore" ) .'"';
	$custom_msgs .= ' data-email_not_empty="'. esc_html__( "The email address is required", "zozothemescore" ) .'"';
	$custom_msgs .= ' data-email_valid="'. esc_html__( "The input is not a valid email address", "zozothemescore" ) .'"';
	$custom_msgs .= ' data-phone_valid="'. esc_html__( "The value can contain only digits", "zozothemescore" ) .'"';
	$custom_msgs .= ' data-msg_not_empty="'. esc_html__( "Message is required", "zozothemescore" ) .'"';
	...
	if( isset( $form ) && $form == 'on' ) {
		$output .= '<div class="zozo-contact-form-wrapper">';
			...
			$output .= '<div class="zozo-contact-form-container">';
				$output .= '<form class="zozo-contact-form" id="sc-contactform'.$zozo_contactform_id.'" name="sc-contactform'.$zozo_contactform_id.'" method="post" action="#"'. $custom_msgs .'>';
						
					// Name Field
					$output .= '<div class="zozo-input-text form-group">';
						$output .= '<label class="sr-only" for="contact_name">'.$name.'</label>';
						$output .= '<input type="text" name="contact_name" id="contact_name" class="input-name form-control" value="" placeholder="'.esc_attr($name).'">';
					$output .= '</div>';								
								
					// Email Field
					$output .= '<div class="zozo-input-email form-group">';
						$output .= '<label class="sr-only" for="contact_email">'.$email.'</label>';
						$output .= '<input type="text" name="contact_email" id="contact_email" class="input-email form-control" value="" placeholder="'.esc_attr($email).'">';															
					$output .= '</div>';
								
					// Phone Field
					$output .= '<div class="zozo-input-phone form-group">';									
						$output .= '<label class="sr-only" for="contact_phone">'.$phone.'</label>';
						$output .= '<input type="text" id="contact_phone" name="contact_phone" class="input-phone form-control" value="" placeholder="'.esc_attr($phone).'">';									
					$output .= '</div>';
...
function zozo_fullwidth_box_shortcode( $atts, $content = null ) {
	...
	if( isset( $overlay ) && $overlay == 'yes' ) {
		if( isset( $overlay_color ) && $overlay_color != '' ) {
			$bg_color_rgb = aven_zozo_hex2rgb( $overlay_color );
