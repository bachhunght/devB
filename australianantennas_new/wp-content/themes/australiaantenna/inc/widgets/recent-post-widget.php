...
class recent_posts extends WP_Widget {
    function recent_posts() {
        ...
        /* Create the widget. */
        $this->WP_Widget( 'recent_posts-widget', __('CC Recent Posts Widget', 'wp-rockPalace'), $widget_ops, '' );
    }
...
<div class="widgett">		
    <?php
        ...
        if ( has_post_thumbnail() ){
            $image = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), 'widgetSmall', false);
            $image = $image[0];}	
        $no_comments = get_comments_number( get_the_ID());
        if($no_comments == 0){
            if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { $comment_0 = stripText($pmc_data['translation_comment_no_responce']); } else {  $comment_0 = __('No Responses','wp-rockPalace'); } 
        }
        else if($no_comments == 1){
            if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { $comment_1 = stripText($pmc_data['translation_comment_one_comment']); } else {  $comment_1 = __('One Response','wp-rockPalace'); } 
        }
        else{
            if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { $comment_2 = stripText($pmc_data['translation_comment_max_comment']); } else {  $comment_2 = __('Responses','wp-rockPalace'); } 
        }			

    ?>			<div class="imgholder"><a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title(); ?>">
