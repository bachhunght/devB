...
class pb_backupbuddy_ajax extends pb_backupbuddy_ajaxcore {
	...
	function remote_test() {
		...
		} elseif ( pb_backupbuddy::_POST( '#type' ) == 'ftp' ) {
			if ( pb_backupbuddy::_POST( '#ftps' ) == '0' ) {
				$ftp_type = 'ftp';
			} else {
				$ftp_type = 'ftps';
			}
			if ( true === ( $response = pb_backupbuddy::$classes['core']->test_ftp( pb_backupbuddy::_POST( '#address' ), pb_backupbuddy::_POST( '#username' ), pb_backupbuddy::_POST( '#password' ), pb_backupbuddy::_POST( '#path' ), $ftp_type ) ) ) {
				echo __('Test completed successfully.', 'it-l10n-backupbuddy' );
			} else {
				echo __('Failure', 'it-l10n-backupbuddy' ) . '; ' . $response;
			}
